...
import { Shield, CheckCircle2, XCircle, MapPin, Mail, Phone, FileText, Calendar, User, Briefcase, Image as ImageIcon } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";

export const Route = createFileRoute("/admin")({
  head: () => ({ meta: [{ title: "Verification Queue — Takaz" }] }),
  component: AdminPage,
});
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Verification queue</h1>
          <p className="text-muted-foreground">Review partner applications before they go live.</p>
        </div>
      </div>
              ...
              <motion.div key={active.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  ...
                  {active.status === "pending" && (
                    <div className="flex gap-2">
                      <button disabled={busy} onClick={approve} className="flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-medium text-primary-foreground shadow-[var(--shadow-glow)] hover:scale-[1.02] transition disabled:opacity-50">
                        <CheckCircle2 className="h-3.5 w-3.5" /> Approve with Live DB Push
                      </button>
                ...
                <div className="mt-6 grid gap-6 md:grid-cols-2">
                  <Section title="Personal data" icon={User}>
                    ...
                    <Row icon={Briefcase} label="User ID" value={active.user_id ?? "guest"} />
                  </Section>
                  ...
                  <Section title="Listing attributes" icon={MapPin}>
                    ...
                    <Row icon={FileText} label="Published" value={active.published ? "Yes — visible on public map" : "No"} />
